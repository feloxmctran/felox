  ...
  const StatusBadge = ({ text, color = "emerald", size = "xs" }) => {
    ...
    const sizeCls =
      ...
      size === "sm" ? "text-sm"   :
                      "text-xs";
    return (
      <span className={`inline-flex items-center px-2.5 py-1 rounded-full font-semibold ${sizeCls} ${cls}`}>
        {text}
      </span>
    );
  };
  ...
  async function fetchImportantDay(dayKeyIn) {
    ...
    try {
      ...
      // ÇALIŞAN endpoint: important-day (day param’lı -> sonra paramsız)
      const urls = dayKey
        ? [
      ...
      // Tarihi hazırla (yıl göstermiyoruz)
      const key =
        dayKey ||
        data?.day_key ||
      ...
      // record varsa alanları esnek ve trim’leyerek al
...
{speedTier?.tier ? (
  <div className="mt-1">
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <div className="text-xs text-gray-600 whitespace-nowrap">
        Hızın: {Number(speedTier?.avg_spent_seconds || 0).toFixed(1)} sn
      </div>
    </div>
    <div className="mt-1">
  ...
  <div className="mt-1">
    <div className="flex items-center gap-2 flex-wrap">
      <StatusBadge text="Hesaplanıyor…" color="gray" size="sm" />
...
</div> {/* sağ sütun kapanışı (flex-1 min-w-0) */}
</div> {/* avatar satırı kapanışı (flex items-center ...) */}
  ...
  <div className="mt-1">
    <div className="flex items-center gap-2 flex-wrap">
      <StatusBadge text="Hesaplanıyor…" color="gray" size="sm" />
      <div className="text-xs text-gray-400">Hızın hesaplanıyor…</div>
    </div>
    <div className="mt-1">
      <BookCountPill count={books} />
    </div>
